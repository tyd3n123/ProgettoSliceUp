...
pizzeria1.on('click', function() {
    sessionStorage.setItem("coordinate-pizzeriaLat", JSON.stringify(45.31354101534006));
    sessionStorage.setItem("coordinate-pizzeriaLon", JSON.stringify(9.497417886649474));
...
// Funzione per animare le pizze fluttuanti
document.addEventListener('DOMContentLoaded', function() {
    // Se vuoi aggiungere elementi dinamicamente:
    const pizzas = document.querySelectorAll('.floating-pizza');
    
    // Randomizza ulteriormente le posizioni iniziali
    pizzas.forEach(pizza => {
        const randomLeft = Math.random() * 100;
        pizza.style.left = `${randomLeft}%`;
        
        // Aggiunge un leggero movimento laterale durante l'animazione
        pizza.style.animationDuration = `${15 + Math.random() * 10}s`;
        pizza.style.animationDelay = `-${Math.random() * 20}s`;
    });
});